...
class SchedulingDinamicoAlgorithmOptimizado {
  ...
  SchedulingDinamicoAlgorithmOptimizado({
    ...
    _estadoActual!.todosPasos = pasos.map((p) => PasoSchedulingDinamico(
      id: p.id,
      nombre: p.nombre,
  ...
  /// Encuentra la mejor combinación de recursos disponibles para un paso
  _CombinacionRecursos? _encontrarMejorCombinacion(PasoSchedulingDinamico paso) {
    int tiempoActual = _estadoActual!.tiempoActual;
    // Buscar cocineros y utensilios disponibles del tipo requerido
    final cocineros = _estadoActual!.cocineros.where((c) => c.tipo == paso.tipoCocinero).toList();
    final utensilios = _estadoActual!.utensilios.where((u) => u.tipo == paso.tipoUtensilio).toList();
    for (final cocinero in cocineros) {
      for (final utensilio in utensilios) {
        // Ambos recursos deben estar libres en el tiempo actual
        if (_estaDisponible(cocinero, tiempoActual) && _estaDisponible(utensilio, tiempoActual)) {
          return _CombinacionRecursos(cocinero, utensilio, tiempoActual);
        }
      }
    }
    return null;
  ...
  /// Asigna recursos a un paso y lo pone en ejecución
  void _asignarRecursos(PasoSchedulingDinamico paso, CocineroScheduling cocinero, UtensilioScheduling utensilio) {
    int tiempoActual = _estadoActual!.tiempoActual;
    paso.iniciarEjecucion(tiempoActual);
    // Registrar en el horario de los recursos
    cocinero.horario.add(HorarioItem(
      tiempoInicio: tiempoActual,
      duracion: paso.duracion,
      pasoId: paso.id,
    ));
    utensilio.horario.add(HorarioItem(
      tiempoInicio: tiempoActual,
      duracion: paso.duracion,
      pasoId: paso.id,
    ));
    ...
    _log('🔗 Asignado 24{paso.nombre} a 24{cocinero.nombre} + 24{utensilio.nombre} en T24tiempoActual');
  }
  ...
  bool get _huboProgresoEnIteracionAnterior => false; // Limpieza: ya no se usa variable de progreso
}
...
extension EstadoSchedulingExtension on EstadoScheduling {
  ...
  int? get proximoEvento {
    ...
    return eventosFinPaso.where((e) => e != null).cast<int>().reduce(min);
  }
}

/// Extensión para obtener propiedades adicionales de PasoSchedulingDinamico
extension PasoSchedulingDinamicoGetters on PasoSchedulingDinamico {
  String get cocineroRequerido => tipoCocinero;
  String get utensilioRequerido => tipoUtensilio;
  // Si necesitas cocineroAsignado/utensilioAsignado, defínelos aquí o en la clase
  // Getters para compatibilidad con la UI (pueden retornar null o el valor correcto si lo implementas)
  String? get cocineroAsignado => null;
  String? get utensilioAsignado => null;
}

// --- LIMPIEZA FINAL ---
// Se eliminaron los métodos duplicados, variables y referencias a _tiempoInicio, _ultimosCompletados, _asignarNuevosPasos y cualquier código muerto.
// El resto del algoritmo permanece funcional y compilable.
